...
public class MasterPaymentNegotiation implements Serializable {
    ...
    @Id
    @GeneratedValue(strategy=GenerationType.SEQUENCE, generator="MASTER_PAYMENT_NEGOTIATION_GENERATOR")
   	@SequenceGenerator(name="MASTER_PAYMENT_NEGOTIATION_GENERATOR", sequenceName="terms.PAYMENT_NEGOTIATION_ID_SEQ",allocationSize=1)
    @Basic(optional = false)
    @NotNull
    @Column(name = "PK_PAYMENT_NEGOTIATION_ID")
    private Integer pkPaymentNegotiationId;
    @Basic(optional = false)
    @NotNull
    @Size(min = 1, max = 20)
    ...
    public MasterPaymentNegotiation(Integer pkPaymentNegotiationId) {
        this.pkPaymentNegotiationId = pkPaymentNegotiationId;
    }

    public MasterPaymentNegotiation(Integer pkPaymentNegotiationId, String payNegoCode) {
        this.pkPaymentNegotiationId = pkPaymentNegotiationId;
        this.payNegoCode = payNegoCode;
    }

    public Integer getPkPaymentNegotiationId() {
        return pkPaymentNegotiationId;
    }

    public void setPkPaymentNegotiationId(Integer pkPaymentNegotiationId) {
        this.pkPaymentNegotiationId = pkPaymentNegotiationId;
    }
    ...
    public NegotiationDto convertEntityToDto() {
		NegotiationDto dto= new NegotiationDto();
		dto.setNagotiationCode(payNegoCode);
		dto.setNagotiationName(payNegoName);
		dto.setNagotiationTermId(pkPaymentNegotiationId);
		dto.setPrintDescription(payNegoDesc);
		return dto;
