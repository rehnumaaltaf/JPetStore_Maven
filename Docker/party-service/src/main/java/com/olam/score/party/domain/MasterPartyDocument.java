 ...
 */
@Entity
@Table(name = "MASTER_PARTY_DOCUMENT")
@NamedQueries({
    @NamedQuery(name = "MasterPartyDocument.findAll", query = "SELECT m FROM MasterPartyDocument m")})

@Data
@NoArgsConstructor
@RequiredArgsConstructor
@EqualsAndHashCode(of = { "pkPartyDocumentId" })
@ToString(of = { "pkPartyDocumentId" })
public class MasterPartyDocument implements Serializable {

    private static final long serialVersionUID = 1L;
    @Id
    @Basic(optional = false)
    @GeneratedValue(strategy=GenerationType.SEQUENCE, generator="MASTER_PARTY_DOCUMENT_GENERATOR")
   	@SequenceGenerator(name="MASTER_PARTY_DOCUMENT_GENERATOR", sequenceName="party.PARTY_DOCUMENT_ID_SEQ",allocationSize=1)
    @NotNull
    @NonNull
    @Column(name = "PK_PARTY_DOCUMENT_ID")
    private Integer pkPartyDocumentId;
    @Column(name = "FK_DOCUMENT_REF_ID")
    private Integer fkDocumentRefId;
    @Column(name = "FK_DOCUMENT_TYPE_REF_ID")
