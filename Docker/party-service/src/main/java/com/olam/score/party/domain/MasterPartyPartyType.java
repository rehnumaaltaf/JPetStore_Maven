 ...
 */
@Entity
@Table(name = "MASTER_PARTY_PARTY_TYPE")
@NamedQueries({
    @NamedQuery(name = "MasterPartyPartyType.findAll", query = "SELECT m FROM MasterPartyPartyType m")})
@Data
@NoArgsConstructor
@RequiredArgsConstructor
@EqualsAndHashCode(of = { "pkPartyPartyTypeId" })
@ToString(of = { "pkPartyPartyTypeId" })
public class MasterPartyPartyType implements Serializable {

    private static final long serialVersionUID = 1L;
    @Id
    @Basic(optional = false)
    @GeneratedValue(strategy=GenerationType.SEQUENCE, generator="MASTER_PARTY_PARTY_TYPE_GENERATOR")
   	@SequenceGenerator(name="MASTER_PARTY_PARTY_TYPE_GENERATOR", sequenceName="party.PARTY_PARTY_TYPE_ID_SEQ",allocationSize=1)
    @NotNull
    @NonNull
    @Column(name = "PK_PARTY_PARTY_TYPE_ID")
    private Integer pkPartyPartyTypeId;
    @Size(max = 100)
    ...
    public MasterPartyPartyType(MasterPartyType partyType) {
    	this.setFkPartyTypeId(partyType);
    }
