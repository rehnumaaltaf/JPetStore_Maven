 ...
 */
@Entity
@Table(name = "MASTER_PARTY_CONTACT")
@NamedQueries({
    @NamedQuery(name = "MasterPartyContact.findAll", query = "SELECT m FROM MasterPartyContact m")})
@Data
@NoArgsConstructor
@RequiredArgsConstructor
@EqualsAndHashCode(of = { "pkPartyContactId" })
@ToString(of = { "pkPartyContactId" })

public class MasterPartyContact implements Serializable {

    private static final long serialVersionUID = 1L;
    @Id
    @Basic(optional = false)
    @GeneratedValue(strategy=GenerationType.SEQUENCE, generator="MASTER_PARTY_CONTACT_GENERATOR")
   	@SequenceGenerator(name="MASTER_PARTY_CONTACT_GENERATOR", sequenceName="party.PARTY_CONTACT_ID_SEQ",allocationSize=1)
    @NotNull
    @NonNull
    @Column(name = "PK_PARTY_CONTACT_ID")
    private Integer pkPartyContactId;
    @Size(max = 200)
    @Column(name = "PARTY_CONTACT_FULLNAME")
    private String partyContactFullname;
    @Size(max = 200)
    @Column(name = "PARTY_CONTACT_DESIGNATION")
    private String partyContactDesignation;
    @Size(max = 1000)
    @Column(name = "PARTY_CONTACT_EMAIL")
    private String partyContactEmail;
    @Size(max = 50)
    @Column(name = "PARTY_CONTACT_WORK_PHONE")
    private String partyContactWorkPhone;
    @Size(max = 50)
    @Column(name = "PARTY_CONTACT_MOBILE")
    private String partyContactMobile;
    @Size(max = 50)
    @Column(name = "PARTY_CONTACT_FAX")
    private String partyContactFax;
    @Size(max = 1)
    @Column(name = "PARTY_CONTACT_IS_DEFAULT")
    ...
    @JoinColumn(name = "FK_DEPT_ID", referencedColumnName = "PK_DEPT_ID")
    @ManyToOne
    private MasterDepartment fkDeptId;
